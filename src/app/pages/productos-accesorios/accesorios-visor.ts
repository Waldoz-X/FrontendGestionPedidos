...
import { ProductoAccesorio, VarianteNestedDtoAccesorio, SkuNestedDtoAccesorio } from '../service/productos-accesorios/productos-accesorios-api.types';
    ...
    template: `
        ...
        <div class="card p-6">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Visor de Accesorios</h2>
                    <p class="text-surface-500 dark:text-surface-400 mt-1">Explora visualmente el catálogo de accesorios y variantes.</p>
                </div>
            ...
            @if (loading()) {
                ...
                @if (filteredAccesorios().length === 0) {
                    <div class="text-center py-12 border rounded-lg border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900">
                        ...
                        <p class="text-surface-600 dark:text-surface-400 text-lg font-semibold">No se encontraron accesorios que coincidan con la búsqueda.</p>
                    </div>
                } @else {
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @for (accesorio of paginatedAccesorios(); track accesorio.clProducto) {
                            <div 
                            ...
                            >
                                ...
                                <div class="relative bg-surface-100 dark:bg-surface-955 aspect-square flex items-center justify-center overflow-hidden border-b border-surface-150 dark:border-surface-850">
                                    <img 
                                        [src]="getImagenMini(accesorio)" 
                                        [alt]="accesorio.nbProducto" 
                                    ...
                                    <div class="absolute top-3 right-3 flex flex-col gap-1.5">
                                        <p-tag 
                                            [value]="accesorio.clEstatusProducto === 'ACTIVO' ? 'ACTIVO' : 'INACTIVO'" 
                                            [severity]="accesorio.clEstatusProducto === 'ACTIVO' ? 'success' : 'danger'"
                                        />
                                        <p-tag 
                                            [value]="getCategoriaLabel(accesorio.idElemCategoria)" 
                                            severity="secondary"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider">{{ accesorio.clProducto }}</span>
                                    <h3 class="text-lg font-bold text-surface-800 dark:text-surface-100 mt-1 line-clamp-1">{{ accesorio.nbProducto }}</h3>
                                    ...
                                    <div class="flex flex-col gap-2 mt-3 text-xs flex-grow">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5 line-clamp-2">{{ getColoresDisponibles(accesorio) }}</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-surface-400 dark:text-surface-500 font-bold uppercase tracking-wider">Tallas</span>
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5">{{ getTallasDisponibles(accesorio) }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="border-t border-surface-100 dark:border-surface-800 pt-3 mt-4 flex items-center justify-between">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold line-clamp-1">{{ getColeccionLabel(accesorio.idElemLineaColeccion) }}</span>
                                        </div>
                                        <div class="flex flex-col items-end">
                                            <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Stock Total</span>
                                            <p-tag 
                                                [value]="getStockTotal(accesorio) > 0 ? getStockTotal(accesorio) + ' pzs' : 'AGOTADO'"
                                                [severity]="getStockTotal(accesorio) > 0 ? (getStockTotal(accesorio) > 10 ? 'info' : 'warn') : 'danger'"
                    ...
                    <div class="mt-6 flex justify-center">
                        <p-paginator 
                            [first]="first()" 
                            [rows]="rows()" 
                            [totalRecords]="filteredAccesorios().length" 
        ...
        <p-dialog
            ...
            [style]="{ width: '750px' }"
            [modal]="true"
            [header]="accesorioSeleccionado()?.nbProducto || ''"
            [resizable]="false"
            [draggable]="false"
            styleClass="p-fluid rounded-2xl"
        >
            <ng-template #content>
                @if (accesorioSeleccionado(); as accesorio) {
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4 pb-2">
                        ...
                        <div class="col-span-12 md:col-span-6 flex flex-col gap-5">
                            <div>
                                <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider block">{{ accesorio.clProducto }}</span>
                                <h3 class="text-2xl font-extrabold text-surface-900 dark:text-surface-50 mt-1">{{ accesorio.nbProducto }}</h3>
                                <p class="text-sm text-surface-500 mt-2 line-clamp-3 italic">
                                    {{ accesorio.dsMaterialPrincipal || 'Sin descripción técnica disponible.' }}
                                </p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 bg-surface-50 dark:bg-surface-955 p-4 rounded-xl border border-surface-200/60 dark:border-surface-800">
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Subcategoría:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ accesorio.clSubcategoria || 'N/A' }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Material:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ accesorio.dsMaterialPrincipal || 'N/A' }}</span>
                                </div>
                            </div>
...
export class AccesoriosVisor implements OnInit {
    private readonly apiService = inject(ProductosAccesorioApiService);
    private readonly catalogosService = inject(CatalogosApiService);
    private readonly messageService = inject(MessageService);
    private readonly destroyRef = inject(DestroyRef);

    // --- State ---
    accesorios = signal<ProductoAccesorio[]>([]);
    ...
    accesorioSeleccionado = signal<ProductoAccesorio | null>(null);
    varianteIdx = signal<number>(0);

    // --- Computed Filter ---
    filteredAccesorios = computed(() => {
    ...
    paginatedAccesorios = computed(() => {
        const list = this.filteredAccesorios();
    ...
    varianteSeleccionada = computed(() => {
        const accesorio = this.accesorioSeleccionado();
        if (!accesorio || !accesorio.variantes || accesorio.variantes.length === 0) return null;
        const index = this.varianteIdx();
        return accesorio.variantes[index] || accesorio.variantes[0];
    });
    ...
    cargarCatalogosMaestros(): void {
        this.apiService.getProductosAccesorio().pipe(
    ...
    getImagenMini(accesorio: ProductoAccesorio): string {
        if (accesorio.variantes && accesorio.variantes.length > 0) {
            const url = accesorio.variantes[0].urlImagen;
    ...
    getColoresDisponibles(accesorio: ProductoAccesorio): string {
        if (!accesorio.variantes || accesorio.variantes.length === 0) return 'N/A';
        return accesorio.variantes
    ...
    getTallasDisponibles(accesorio: ProductoAccesorio): string {
        if (!accesorio.variantes || accesorio.variantes.length === 0) return 'N/A';
        const allTallas: string[] = [];
        accesorio.variantes.forEach(v => {
            (v.skus || []).forEach(s => {
    ...
    getStockTotal(accesorio: ProductoAccesorio): number {
        let total = 0;
        (accesorio.variantes || []).forEach(v => {
            (v.skus || []).forEach(s => {
    ...
    abrirDetalle(accesorio: ProductoAccesorio): void {
        this.accesorioSeleccionado.set(accesorio);
