...
import { ProductoCono, VarianteNestedDtoCono, SkuNestedDtoCono } from '../service/productos-cono/productos-cono-api.types';
import { ProductosConoApiService } from '../service/productos-cono/productos-cono-api.service';
import { CatalogosApiService, CatalogoElemento } from '../service/catalogos-api.service';

@Component({
    selector: 'app-conos-visor',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
    ...
    template: `
        ...
        <div class="card p-6">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Visor de Conos</h2>
                    <p class="text-surface-500 dark:text-surface-400 mt-1">Explora visualmente el catálogo de conos y variantes.</p>
                </div>
            ...
            @if (loading()) {
                ...
                @if (filteredConos().length === 0) {
                    <div class="text-center py-12 border rounded-lg border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900">
                        ...
                        <p class="text-surface-600 dark:text-surface-400 text-lg font-semibold">No se encontraron conos que coincidan con la búsqueda.</p>
                    </div>
                } @else {
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @for (cono of paginatedConos(); track cono.clProducto) {
                            <div 
                            ...
                            >
                                ...
                                <div class="relative bg-surface-100 dark:bg-surface-955 aspect-square flex items-center justify-center overflow-hidden border-b border-surface-150 dark:border-surface-850">
                                    ...
                                    <div class="absolute top-3 right-3 flex flex-col gap-1.5">
                                        <p-tag 
                                            [value]="cono.clEstatusProducto === 'ACTIVO' ? 'ACTIVO' : 'INACTIVO'" 
                                            [severity]="cono.clEstatusProducto === 'ACTIVO' ? 'success' : 'danger'"
                                        />
                                        <p-tag 
                                            [value]="getCategoriaLabel(cono.idElemCategoria)" 
                                            severity="secondary"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider">{{ cono.clProducto }}</span>
                                    <h3 class="text-lg font-bold text-surface-800 dark:text-surface-100 mt-1 line-clamp-1">{{ cono.nbProducto }}</h3>
                                    ...
                                    <div class="flex flex-col gap-2 mt-3 text-xs flex-grow">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5 line-clamp-2">{{ getColoresDisponibles(cono) }}</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-surface-400 dark:text-surface-500 font-bold uppercase tracking-wider">Tallas</span>
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5">{{ getTallasDisponibles(cono) }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="border-t border-surface-100 dark:border-surface-800 pt-3 mt-4 flex items-center justify-between">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold line-clamp-1">{{ getColeccionLabel(cono.idElemLineaColeccion) }}</span>
                                        </div>
                                        <div class="flex flex-col items-end">
                                            <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Stock Total</span>
                                            <p-tag 
                                                [value]="getStockTotal(cono) > 0 ? getStockTotal(cono) + ' pzs' : 'AGOTADO'"
                                                [severity]="getStockTotal(cono) > 0 ? (getStockTotal(cono) > 10 ? 'info' : 'warn') : 'danger'"
                    ...
                    <div class="mt-6 flex justify-center">
                        <p-paginator 
                            [first]="first()" 
                            [rows]="rows()" 
                            [totalRecords]="filteredConos().length" 
        ...
        <p-dialog
            ...
            [style]="{ width: '750px' }"
            [modal]="true"
            [header]="conoSeleccionado()?.nbProducto || ''"
            [resizable]="false"
            [draggable]="false"
            styleClass="p-fluid rounded-2xl"
        >
            <ng-template #content>
                @if (conoSeleccionado(); as cono) {
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4 pb-2">
                        ...
                        <div class="col-span-12 md:col-span-6 flex flex-col gap-5">
                            <div>
                                <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider block">{{ cono.clProducto }}</span>
                                <h3 class="text-2xl font-extrabold text-surface-900 dark:text-surface-50 mt-1">{{ cono.nbProducto }}</h3>
                                <p class="text-sm text-surface-500 mt-2 line-clamp-3 italic">
                                    {{ cono.dsMaterial || 'Sin descripción técnica disponible.' }}
                                </p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 bg-surface-50 dark:bg-surface-955 p-4 rounded-xl border border-surface-200/60 dark:border-surface-800">
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Altura:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ cono.noAlturaCm || '0' }} cm</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Material:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ cono.dsMaterial || 'N/A' }}</span>
                                </div>
                            </div>
...
export class ConosVisor implements OnInit {
    private readonly apiService = inject(ProductosConoApiService);
    private readonly catalogosService = inject(CatalogosApiService);
    private readonly messageService = inject(MessageService);
    private readonly destroyRef = inject(DestroyRef);

    // --- State ---
    conos = signal<ProductoCono[]>([]);
    ...
    conoSeleccionado = signal<ProductoCono | null>(null);
    ...
    cargarCatalogosMaestros(): void {
        this.apiService.getProductosCono().pipe(
            ...
            error: (err: HttpErrorResponse) => {
                this.loading.set(false);
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'No se pudieron cargar los conos.', life: 5000 });
            }
        });
    }
    ...
    abrirDetalle(cono: ProductoCono): void {
        this.conoSeleccionado.set(cono);
