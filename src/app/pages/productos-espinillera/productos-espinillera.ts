...
@Component({
    ...
    template: `
        ...
        <p-dialog 
            ...
            [style]="{ width: '500px' }" 
            header="Carga Masiva de Espinilleras" 
            [modal]="true"
        >
            <ng-template #content>
        ...
        <div class="card">
            <p-toolbar styleClass="mb-6">
                <ng-template #start>
                    <div>
                        <div class="font-semibold text-xl">Productos Espinillera</div>
                        <p class="m-0 text-surface-500 dark:text-surface-400">Gestión de productos espinillera en el sistema.</p>
                    </div>
                </ng-template>
                <ng-template #end>
            ...
            <p-table
                ...
                currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} espinilleras"
            ...
            >
                ...
                <ng-template #emptymessage>
                    <tr>
                        <td colspan="6">No hay espinilleras para mostrar.</td>
                    </tr>
                </ng-template>
            </p-table>
        </div>
        ...
        <p-dialog
            ...
            [style]="{ width: '900px' }"
            [header]="editando() ? 'Editar Espinillera' : 'Nuevo Espinillera'"
            [modal]="true"
        >
            <ng-template #content>
                <p-tabs value="0">
...
export class ProductosEspinillera implements OnInit {
    ...
    // ─── State ───
    productos = signal<ProductoEspinillera[]>([]);
    ...
    datosFormulario: CrearProductoEspinilleraRequest = {
        clProducto: '',
        nbProducto: '',
        idElemCategoria: 0,
            ...
            this.catalogosService.getElementos(clCatalogo).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
                next: (data: any[]) => {
                    ...
                    if (clCatalogo === 'TALLAS') {
                        // Filtrar para que solo muestre las tallas que sean hijas de Espinillera Infantil o Espinillera Adulto
                        mapData = mapData.filter(item => 
                            item.parent && item.parent.toString().toLowerCase().includes('espinillera')
                        );
                    }
    ...
    editarProducto(prod: ProductoEspinillera): void {
        ...
        this.loading.set(true);
        this.apiService.getProductoEspinillera(id).pipe(
            takeUntilDestroyed(this.destroyRef)
        ).subscribe({
            next: (fullProd: any) => {
                this.loading.set(false);
            ...
            error: (err: any) => {
                ...
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar la información detallada del espinillera.', life: 5000 });
            }
        });
    }
    ...
    guardar(): void {
        ...
        const request$ = this.editando() 
            ? this.apiService.actualizarProductoEspinillera(this.idEditando, this.datosFormulario)
            : this.apiService.crearProductoEspinillera(this.datosFormulario);
    ...
    confirmarEliminar(prod: ProductoEspinillera): void {
        this.confirmationService.confirm({
            message: '¿Eliminar el espinillera ' + prod.nbProducto + '?',
            ...
            accept: () => {
                ...
                this.apiService.eliminarProductoEspinillera(id).pipe(
                    ...
                    next: () => {
                        this.cargarProductos();
                        this.messageService.add({ severity: 'success', summary: 'Eliminado', detail: 'Espinillera eliminado correctamente.' });
                    },
