...
import { ProductoEspinillera, VarianteNestedDtoEspinillera, SkuNestedDtoEspinillera } from '../service/productos-espinillera/productos-espinillera-api.types';
...
@Component({
    selector: 'app-espinilleras-visor',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
    ...
    template: `
        ...
        <div class="card p-6">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Visor de Espinilleras</h2>
                    <p class="text-surface-500 dark:text-surface-400 mt-1">Explora visualmente el catálogo de espinilleras y variantes.</p>
                </div>
            ...
            @if (loading()) {
                ...
                @if (filteredEspinilleras().length === 0) {
                    <div class="text-center py-12 border rounded-lg border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900">
                        ...
                        <p class="text-surface-600 dark:text-surface-400 text-lg font-semibold">No se encontraron espinilleras que coincidan con la búsqueda.</p>
                    </div>
                } @else {
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @for (espinillera of paginatedEspinilleras(); track espinillera.clProducto) {
                            <div 
                                ...
                                (click)="abrirDetalle(espinillera)"
                            >
                                ...
                                <div class="relative bg-surface-100 dark:bg-surface-955 aspect-square flex items-center justify-center overflow-hidden border-b border-surface-150 dark:border-surface-850">
                                    <img 
                                        [src]="getImagenMini(espinillera)" 
                                        [alt]="espinillera.nbProducto" 
                                    ...
                                    <div class="absolute top-3 right-3 flex flex-col gap-1.5">
                                        <p-tag 
                                            [value]="espinillera.clEstatusProducto === 'ACTIVO' ? 'ACTIVO' : 'INACTIVO'" 
                                            [severity]="espinillera.clEstatusProducto === 'ACTIVO' ? 'success' : 'danger'"
                                        />
                                        <p-tag 
                                            [value]="getCategoriaLabel(espinillera.idElemCategoria)" 
                                            severity="secondary"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider">{{ espinillera.clProducto }}</span>
                                    <h3 class="text-lg font-bold text-surface-800 dark:text-surface-100 mt-1 line-clamp-1">{{ espinillera.nbProducto }}</h3>
                                    ...
                                    <div class="flex flex-col gap-2 mt-3 text-xs flex-grow">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5 line-clamp-2">{{ getColoresDisponibles(espinillera) }}</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-surface-400 dark:text-surface-500 font-bold uppercase tracking-wider">Tallas</span>
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5">{{ getTallasDisponibles(espinillera) }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="border-t border-surface-100 dark:border-surface-800 pt-3 mt-4 flex items-center justify-between">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold line-clamp-1">{{ getColeccionLabel(espinillera.idElemLineaColeccion) }}</span>
                                        </div>
                                        <div class="flex flex-col items-end">
                                            <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Stock Total</span>
                                            <p-tag 
                                                [value]="getStockTotal(espinillera) > 0 ? getStockTotal(espinillera) + ' pzs' : 'AGOTADO'"
                                                [severity]="getStockTotal(espinillera) > 0 ? (getStockTotal(espinillera) > 10 ? 'info' : 'warn') : 'danger'"
                    ...
                    <div class="mt-6 flex justify-center">
                        <p-paginator 
                            [first]="first()" 
                            [rows]="rows()" 
                            [totalRecords]="filteredEspinilleras().length" 
        ...
        <p-dialog
            ...
            [style]="{ width: '750px' }"
            [modal]="true"
            [header]="espinilleraSeleccionado()?.nbProducto || ''"
            [resizable]="false"
            [draggable]="false"
            styleClass="p-fluid rounded-2xl"
        >
            <ng-template #content>
                @if (espinilleraSeleccionado(); as espinillera) {
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4 pb-2">
                        ...
                        <div class="col-span-12 md:col-span-6 flex flex-col gap-5">
                            <div>
                                <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider block">{{ espinillera.clProducto }}</span>
                                <h3 class="text-2xl font-extrabold text-surface-900 dark:text-surface-50 mt-1">{{ espinillera.nbProducto }}</h3>
                            ...
                            <div class="grid grid-cols-2 gap-3 bg-surface-50 dark:bg-surface-955 p-4 rounded-xl border border-surface-200/60 dark:border-surface-800">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300"></span>
                                </div>
                                <div class="flex flex-col">
...
export class EspinillerasVisor implements OnInit {
    private readonly apiService = inject(ProductosEspinilleraApiService);
    private readonly catalogosService = inject(CatalogosApiService);
    private readonly messageService = inject(MessageService);
    private readonly destroyRef = inject(DestroyRef);

    // --- State ---
    espinilleras = signal<ProductoEspinillera[]>([]);
    ...
    espinilleraSeleccionado = signal<ProductoEspinillera | null>(null);
    varianteIdx = signal<number>(0);

    // --- Computed Filter ---
    filteredEspinilleras = computed(() => {
        const query = this.searchQuery().toLowerCase().trim();
        const list = this.espinilleras();
        ...
        return list.filter(g => {
            ...
            const matchesPalma = g.dsMaterial?.toLowerCase().includes(query);
    ...
    paginatedEspinilleras = computed(() => {
        const list = this.filteredEspinilleras();
    ...
    varianteSeleccionada = computed(() => {
        const espinillera = this.espinilleraSeleccionado();

        if (!espinillera || !espinillera.variantes || espinillera.variantes.length === 0) return null;
        const index = this.varianteIdx();

        return espinillera.variantes[index] || espinillera.variantes[0];
    });
    ...
    cargarCatalogosMaestros(): void {
        this.apiService.getProductosEspinillera().pipe(
            takeUntilDestroyed(this.destroyRef)
        ).subscribe({
            next: (data) => {
                this.espinilleras.set(data);
    ...
    getImagenMini(espinillera: ProductoEspinillera): string {
        if (espinillera.variantes && espinillera.variantes.length > 0) {
            const url = espinillera.variantes[0].urlImagen;
    ...
    getColoresDisponibles(espinillera: ProductoEspinillera): string {
        if (!espinillera.variantes || espinillera.variantes.length === 0) return 'N/A';

        return espinillera.variantes
    ...
    getTallasDisponibles(espinillera: ProductoEspinillera): string {
        if (!espinillera.variantes || espinillera.variantes.length === 0) return 'N/A';
        const allTallas: string[] = [];

        espinillera.variantes.forEach(v => {
            (v.skus || []).forEach(s => {
    ...
    getStockTotal(espinillera: ProductoEspinillera): number {
        let total = 0;
    ...
    abrirDetalle(espinillera: ProductoEspinillera): void {
        this.espinilleraSeleccionado.set(espinillera);
