...
import { ProductoTextil, VarianteNestedDtoTextil, SkuNestedDtoTextil } from '../service/productos-textil/productos-textil-api.types';
    ...
    template: `
        ...
        <div class="card p-6">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Visor de Textils</h2>
                    <p class="text-surface-500 dark:text-surface-400 mt-1">Explora visualmente el catálogo de textils y variantes.</p>
                </div>
            ...
            @if (loading()) {
                ...
                @if (filteredTextils().length === 0) {
                    <div class="text-center py-12 border rounded-lg border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900">
                        ...
                        <p class="text-surface-600 dark:text-surface-400 text-lg font-semibold">No se encontraron textils que coincidan con la búsqueda.</p>
                    </div>
                } @else {
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @for (textil of paginatedTextils(); track textil.id || textil.clProducto) {
                            <div 
                            ...
                            >
                                ...
                                <div class="relative bg-surface-100 dark:bg-surface-955 aspect-square flex items-center justify-center overflow-hidden border-b border-surface-150 dark:border-surface-850">
                                    <img 
                                        [src]="getImagenMini(textil)" 
                                        [alt]="textil.nbProducto" 
                                    ...
                                    <div class="absolute top-3 right-3 flex flex-col gap-1.5">
                                        <p-tag 
                                            [value]="textil.clEstatusProducto === 'ACTIVO' ? 'ACTIVO' : 'INACTIVO'" 
                                            [severity]="textil.clEstatusProducto === 'ACTIVO' ? 'success' : 'danger'"
                                        />
                                        <p-tag 
                                            [value]="getCategoriaLabel(textil.idElemCategoria)" 
                                            severity="secondary"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider">{{ textil.clProducto }}</span>
                                    <h3 class="text-lg font-bold text-surface-800 dark:text-surface-100 mt-1 line-clamp-1">{{ textil.nbProducto }}</h3>
                                    ...
                                    <div class="flex flex-col gap-2 mt-3 text-xs flex-grow">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5 line-clamp-2">{{ getColoresDisponibles(textil) }}</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-surface-400 dark:text-surface-500 font-bold uppercase tracking-wider">Tallas</span>
                                            <span class="text-sm font-semibold text-surface-800 dark:text-surface-200 mt-0.5">{{ getTallasDisponibles(textil) }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="border-t border-surface-100 dark:border-surface-800 pt-3 mt-4 flex items-center justify-between">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-sm font-semibold line-clamp-1">{{ getColeccionLabel(textil.idElemLineaColeccion) }}</span>
                                        </div>
                                        <div class="flex flex-col items-end">
                                            <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Stock Total</span>
                                            <p-tag 
                                                [value]="getStockTotal(textil) > 0 ? getStockTotal(textil) + ' pzs' : 'AGOTADO'"
                                                [severity]="getStockTotal(textil) > 0 ? (getStockTotal(textil) > 10 ? 'info' : 'warn') : 'danger'"
                    ...
                    <div class="mt-6 flex justify-center">
                        <p-paginator 
                            [first]="first()" 
                            [rows]="rows()" 
                            [totalRecords]="filteredTextils().length" 
        ...
        <p-dialog
            ...
            [style]="{ width: '750px' }"
            [modal]="true"
            [header]="textilSeleccionado()?.nbProducto || ''"
            [resizable]="false"
            [draggable]="false"
            styleClass="p-fluid rounded-2xl"
        >
            <ng-template #content>
                @if (textilSeleccionado(); as textil) {
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4 pb-2">
                        ...
                        <div class="col-span-12 md:col-span-6 flex flex-col gap-5">
                            <div>
                                <span class="text-xs font-mono font-bold text-primary dark:text-primary-400 uppercase tracking-wider block">{{ textil.clProducto }}</span>
                                <h3 class="text-2xl font-extrabold text-surface-900 dark:text-surface-50 mt-1">{{ textil.nbProducto }}</h3>
                                <p class="text-sm text-surface-500 mt-2 line-clamp-3 italic">
                                    {{ textil.dsComposicion || 'Sin descripción técnica disponible.' }}
                                </p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 bg-surface-50 dark:bg-surface-955 p-4 rounded-xl border border-surface-200/60 dark:border-surface-800">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ textil.nbTejido || 'N/A' }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Índice Palma:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ textil.dsCorte || 'N/A' }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Cierre:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ textil.idElemGenero || 'N/A' }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs text-surface-400 dark:text-surface-505 font-medium">Forro:</span>
                                    <span class="text-sm font-bold text-surface-700 dark:text-surface-300">{{ textil.noGramajeGsm || 'N/A' }}</span>
                                </div>
                            </div>
...
export class TextilesVisor implements OnInit {
    ...
    textilSeleccionado = signal<ProductoTextil | null>(null);
    varianteIdx = signal<number>(0);

    // --- Computed Filter ---
    filteredTextils = computed(() => {
        const query = this.searchQuery().toLowerCase().trim();
        const list = this.textils();
        ...
        return list.filter(g => {
            ...
            const matchesPalma = g.nbTejido?.toLowerCase().includes(query);
    ...
    paginatedTextils = computed(() => {
        const list = this.filteredTextils();
    ...
    varianteSeleccionada = computed(() => {
        const textil = this.textilSeleccionado();
        if (!textil || !textil.variantes || textil.variantes.length === 0) return null;
        const index = this.varianteIdx();
        return textil.variantes[index] || textil.variantes[0];
    });
    ...
    getImagenMini(textil: ProductoTextil): string {
        if (textil.variantes && textil.variantes.length > 0) {
            const url = textil.variantes[0].urlImagen;
    ...
    getColoresDisponibles(textil: ProductoTextil): string {
        if (!textil.variantes || textil.variantes.length === 0) return 'N/A';
        return textil.variantes
    ...
    getTallasDisponibles(textil: ProductoTextil): string {
        if (!textil.variantes || textil.variantes.length === 0) return 'N/A';
        const allTallas: string[] = [];
        textil.variantes.forEach(v => {
            (v.skus || []).forEach(s => {
    ...
    getStockTotal(textil: ProductoTextil): number {
        let total = 0;
        (textil.variantes || []).forEach(v => {
            (v.skus || []).forEach(s => {
    ...
    abrirDetalle(textil: ProductoTextil): void {
        this.textilSeleccionado.set(textil);
