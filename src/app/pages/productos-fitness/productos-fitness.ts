...
@Component({
    ...
    template: `
        ...
        <p-dialog 
            ...
            [style]="{ width: '500px' }" 
            header="Carga Masiva de Fitnesss" 
            [modal]="true"
        >
            <ng-template #content>
        ...
        <div class="card">
            <p-toolbar styleClass="mb-6">
                <ng-template #start>
                    <div>
                        <div class="font-semibold text-xl">Productos Fitness</div>
                        <p class="m-0 text-surface-500 dark:text-surface-400">Gestión de productos fitness en el sistema.</p>
                    </div>
                </ng-template>
                <ng-template #end>
            ...
            <p-table
                ...
                currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} fitness"
            ...
            >
                ...
                <ng-template #emptymessage>
                    <tr>
                        <td colspan="6">No hay fitness para mostrar.</td>
                    </tr>
                </ng-template>
            </p-table>
        </div>
        ...
        <p-dialog
            ...
            [style]="{ width: '900px' }"
            [header]="editando() ? 'Editar Fitness' : 'Nuevo Fitness'"
            [modal]="true"
        >
            <ng-template #content>
                <p-tabs value="0">
                            ...
                            <div class="flex flex-col gap-6 pt-4">
                                <div class="grid grid-cols-12 gap-4">
                                    ...
                                    <div class="col-span-6 md:col-span-4">
                                        ...
                                        <input type="text" pInputText [(ngModel)]="datosFormulario.dsComposicion" fluid />
                                    </div>
                                    <div class="col-span-6 md:col-span-4">
                                        <label class="block font-bold mb-3">Relleno</label>
                                        <input type="text" pInputText [(ngModel)]="datosFormulario.dsRelleno" fluid />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-12 gap-4">
                                    ...
                                    <div class="col-span-12 md:col-span-6">
                                        ...
                                        <input type="text" pInputText [(ngModel)]="datosFormulario.dsCierre" fluid />
                                    </div>
                                    <div class="col-span-12 md:col-span-6">
                                        <label class="block font-bold mb-3">Protección</label>
                                        <input type="text" pInputText [(ngModel)]="datosFormulario.dsProteccion" fluid />
                                    </div>
                                    <div class="col-span-12 md:col-span-6">
                                        <label class="block font-bold mb-3">Peso (Oz)</label>
                                        <input type="number" pInputText [(ngModel)]="datosFormulario.noPesoOz" fluid />
                                    </div>
                                </div>
                            </div>
                        </p-tabpanel>
...
export class ProductosFitness implements OnInit {
    ...
    // ─── State ───
    productos = signal<ProductoFitness[]>([]);
    ...
    datosFormulario: CrearProductoFitnessRequest = {
        clProducto: '',
        nbProducto: '',
        idElemCategoria: 0,
            ...
            this.catalogosService.getElementos(clCatalogo).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
                next: (data: any[]) => {
                    ...
                    if (clCatalogo === 'TALLAS') {
                        // Filtrar para que solo muestre las tallas que sean hijas de Fitness Infantil o Fitness Adulto
                        mapData = mapData.filter(item => 
                            item.parent && item.parent.toString().toLowerCase().includes('fitness')
                        );
                    }
    ...
    editarProducto(prod: ProductoFitness): void {
        const id = (prod as any).idProducto;

        if (!id) return;

        this.loading.set(true);
        this.apiService.getProductoFitness(id).pipe(
            takeUntilDestroyed(this.destroyRef)
        ).subscribe({
            next: (fullProd: any) => {
                this.loading.set(false);
        ...
        noPesoOz: fullProd.noPesoOz || fullProd.NoPesoOz || 0,
            ...
            error: (err: any) => {
                ...
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar la información detallada del fitness.', life: 5000 });
            }
        });
    }
    ...
    guardar(): void {
        ...
        const request$ = this.editando() 
            ? this.apiService.actualizarProductoFitness(this.idEditando, this.datosFormulario)
            : this.apiService.crearProductoFitness(this.datosFormulario);
    ...
    confirmarEliminar(prod: ProductoFitness): void {
        this.confirmationService.confirm({
            message: '¿Eliminar el fitness ' + prod.nbProducto + '?',
            ...
            accept: () => {
                ...
                this.apiService.eliminarProductoFitness(id).pipe(
                    takeUntilDestroyed(this.destroyRef)
                ).subscribe({
                    next: () => {
                        this.cargarProductos();
                        this.messageService.add({ severity: 'success', summary: 'Eliminado', detail: 'Fitness eliminado correctamente.' });
                    },
